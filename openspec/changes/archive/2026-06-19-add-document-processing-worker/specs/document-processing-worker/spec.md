...
The system SHALL store raw extracted text and normalized text in PostgreSQL.

#### Scenario: Document processing succeeds
- **WHEN** text extraction or OCR completes for a supported document
- **THEN** the system stores document-level raw text, document-level normalized text, and page-level text records

#### Scenario: Text is normalized
- **WHEN** raw text is stored
- **THEN** the normalized text removes avoidable OCR/extraction noise such as inconsistent line endings, repeated whitespace, excessive blank lines, and broken hyphenation
